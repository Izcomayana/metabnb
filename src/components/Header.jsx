    ...
    <>
      <div className='shadow-md w-full top-0 left-0'>
        <div className='md:flex items-center justify-between bg-white py-7 md:px-10 px-7 mb-10 md:mb-20'>
          ...
          <ul className={`md:flex md:items-center md:pb-0 pb-12 absolute md:bg-white bg-black opacity-80
          ...
          md:pl-0 pl-9 transition-all duration-700 ease-in ${open ? 'top-20 ':'top-[-490px]'}`}>
            {
              Links.map((link)=>(
                <li key={link.name} className='md:ml-8 text-xl md:my-0 my-5'>
                  <a href={link.link} className='text-white md:text-black md:hover:text-gray-700 hover:text-slate-300 duration-500'>{link.name}</a>
                </li>
              ))
            }
